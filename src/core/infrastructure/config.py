...
class Settings(BaseSettings):
    ...
    def validate_environment_and_secrets(self) -> "Settings":
        ...
        if self.GOVSEC_ENV in ("staging", "production"):
            ...
            insecure_secret_pattern = "".join(["super-secret-", "govsec-key-", "change-in-production"])
            if insecure_secret_pattern == self.GOVSEC_JWT_SECRET or len(self.GOVSEC_JWT_SECRET) < 32:
                raise ValueError(
