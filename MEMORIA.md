...
- **2026-07-29T15:32:00Z (IA Assistente):** Fechamento final e homologação da Capability M2 com validação dos 5 requisitos: (1) `deploy/alertmanager/alertmanager.rendered.yml` removido do índice do Git via `git rm --cached` (`git ls-files` retorna 0 linhas); (2) Seleção real da configuração por ambiente no `docker-compose.yml` via `${GOVSEC_ALERTMANAGER_CONFIG:-...}` com proibições estritas de `test-receiver` em `staging`/`production`; (3) Variável PagerDuty padronizada globalmente para `GOVSEC_PAGERDUTY_SERVICE_KEY` em `.env.example`, `configs/dev/.env.example`, `config.py`, `render_alertmanager_config.py` e `README.md`; (4) Suíte de testes automatizados expandida para 73 testes cobrindo todas as invariantes e regras do Alertmanager; (5) Todos os 7 comandos de validação executados e aprovados com 100% de sucesso (73/73 pytest passed, 0 ruff errors, promtool SUCCESS, amtool SUCCESS, docker compose healthy, fire drill synthetic alert 200 OK). Capability M2 **100% CONCLUÍDA**.
